...
@section('content')
    <div class="min-h-screen bg-[#0f172a] p-6 text-white">
        ...
        <!-- Form Card -->
        <div class="max-w-3xl mx-auto rounded-3xl border border-gray-800 bg-[#111827] p-8 shadow-2xl">

            <form action="{{ route('category.update',$category->id) }}" class="space-y-6" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!-- Category Name -->
                <div>
                    ...
                    <input type="text" value="{{ $category->name }}" placeholder="Enter category name" name="name"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-5">
                        <div
                            class="col-span-3 rounded-2xl border-2 border-dashed border-gray-700 bg-[#1f2937] p-8 text-center hover:border-orange-500">
                        ...
                        <div class="col-span-1">
                            <img src="http://localhost:8000/storage/{{ $category->image }}" alt="{{ $category->name }}"
                                class="rounded-2xl w-80 object-cover">
                        </div>
                    </div>
                </div>
